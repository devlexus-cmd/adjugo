"""sync schéma — base de connaissances (knowledge_docs, knowledge_chunks)

Nouvelle passe de synchro idempotente (create_all + ADD COLUMN manquantes) pour
créer les tables ajoutées après la migration b7d9e1f3a2c4 (base de connaissances
RAG). Non destructif. Même logique que b7d9e1f3a2c4 (cf. app/core/dbsync.py).
...
def upgrade():
    ...
    Base.metadata.create_all(bind=bind)  # crée les tables manquantes (checkfirst)
